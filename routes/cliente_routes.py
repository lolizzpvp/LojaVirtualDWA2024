...
async def get_carrinho(request: Request):
    ...
    return templates.TemplateResponse(
        "pages/carrinho.html",
        {"request": request, "itens": itens_pedido, "valor_total": pedido_carrinho.valor_total},
    )
...
async def get_pagamento(request: Request, id_pedido: int = Path(...)):
    ...
    preference = {
        ...
        "payer": {
            "name": "Test",
            "surname": "Test",
            "email": "test_user_32838962@testuser.com",
        },
        "back_urls": {
...
@router.post("/post_remover_item", reponse_class=HTMLResponse)
async def post_remover_item(id_produto: int = Form(0)):
    if not id_produto:
